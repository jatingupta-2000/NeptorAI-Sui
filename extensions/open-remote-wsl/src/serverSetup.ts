...
export interface ServerInstallOptions {
	...
	commit: string;
	version: string;
	release?: string; // Neptor-specific
	extensionIds: string[];
	envVariables: string[];
...
const DEFAULT_DOWNLOAD_URL_TEMPLATE = 'https://github.com/neptorai/binaries/releases/download/${version}/neptor-reh-${os}-${arch}-${version}.tar.gz';
...
export async function installCodeServer(wslManager: WSLManager, distroName: string, serverDownloadUrlTemplate: string | undefined, extensionIds: string[], envVariables: string[], logger: Log): Promise<ServerInstallResult> {
	...
	if (exitCode !== 0) {
		throw new ServerInstallError(`Couldn't install Neptor server on remote server, install script returned non-zero exit status`);
	}
