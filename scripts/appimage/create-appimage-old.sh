...
# Check if neptor binary exists in current directory
if [ ! -f "./neptor" ]; then
    echo "Error: neptor binary not found in current directory"
    exit 1
fi

# Check if icon exists
if [ ! -f "./neptor.png" ]; then
    echo "Error: neptor.png icon not found in current directory"
    exit 1
fi
...
cp -v ./neptor "$APP_DIR/usr/bin/"

# Copy the icon to required locations
cp -v ./neptor.png "$APP_DIR/neptor.png"
cp -v ./neptor.png "$APP_DIR/usr/share/icons/hicolor/256x256/apps/neptor.png"

# Copy dependencies with error checking
echo "Copying dependencies..."
for lib in $(ldd ./neptor | grep "=> /" | awk '{print $3}'); do
    if [ -f "$lib" ]; then
...
Type=Application
Categories=Utility;
Comment=Neptor Linux Application
EOF
then
