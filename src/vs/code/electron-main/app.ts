...
import { IMetricsService } from '../../workbench/contrib/neptor/common/metricsService.js';
import { INeptorUpdateService } from '../../workbench/contrib/neptor/common/neptorUpdateService.js';
import { MetricsMainService } from '../../workbench/contrib/neptor/electron-main/metricsMainService.js';
import { NeptorMainUpdateService } from '../../workbench/contrib/neptor/electron-main/neptorUpdateMainService.js';
import { LLMMessageChannel } from '../../workbench/contrib/neptor/electron-main/sendLLMMessageChannel.js';
import { NeptorSCMService } from '../../workbench/contrib/neptor/electron-main/neptorSCMMainService.js';
import { INeptorSCMService } from '../../workbench/contrib/neptor/common/neptorSCMTypes.js';
import { MCPChannel } from '../../workbench/contrib/neptor/electron-main/mcpChannel.js';
/**
...
export class CodeApplication extends Disposable {
	...
	/**
	 * Detect checkout-style installs where `out/` exists next to `src/` even when launch did not
	 * propagate `VSCODE_DEV` into the main process (Dock, GUI runner, some IDE debug configs).
	 * Without this, `environmentMainService.isBuilt` stays true and vscode-file requests get
	 * cancelled before `ProtocolMainService` runs.
	 */
	private isDevelopmentSourcesLayout(): boolean {
		try {
			const root = this.environmentMainService.appRoot;
			return (
				existsSync(join(root, 'package.json'))
				&& existsSync(join(root, 'out', 'bootstrap-window.js'))
	...
	private configureSession(): void {
		...
		const isAllowedVsCodeFileRequest = (details: Electron.OnBeforeRequestListenerDetails): boolean => {
		...
		session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
			...
			if (uri.scheme === Schemas.vscodeFileResource) {
				// OSS from sources: `ProtocolMainService` already restricts filesystem roots.
				// Main-window PID checks here often reject legitimate Chromium submodule fetches for
				// dynamic `import(workbench.desktop.main.js)`, surfacing as ERR_FILE_NOT_FOUND.
				if (!this.environmentMainService.isBuilt || this.isDevelopmentSourcesLayout()) {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Neptor main process services (required for services with a channel for comm between browser and electron-main (node))
		services.set(IMetricsService, new SyncDescriptor(MetricsMainService, undefined, false));
		services.set(INeptorUpdateService, new SyncDescriptor(NeptorMainUpdateService, undefined, false));
		services.set(INeptorSCMService, new SyncDescriptor(NeptorSCMService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Neptor - use loggerChannel as reference
		const metricsChannel = ProxyChannel.fromService(accessor.get(IMetricsService), disposables);
		mainProcessElectronServer.registerChannel('neptor-channel-metrics', metricsChannel);

		const neptorUpdatesChannel = ProxyChannel.fromService(accessor.get(INeptorUpdateService), disposables);
		mainProcessElectronServer.registerChannel('neptor-channel-update', neptorUpdatesChannel);

		const sendLLMMessageChannel = new LLMMessageChannel(accessor.get(IMetricsService));
		mainProcessElectronServer.registerChannel('neptor-channel-llmMessage', sendLLMMessageChannel);

		// Neptor added this
		const neptorSCMChannel = ProxyChannel.fromService(accessor.get(INeptorSCMService), disposables);
		mainProcessElectronServer.registerChannel('neptor-channel-scm', neptorSCMChannel);

		// Neptor added this
		const mcpChannel = new MCPChannel();
		mainProcessElectronServer.registerChannel('neptor-channel-mcp', mcpChannel);
