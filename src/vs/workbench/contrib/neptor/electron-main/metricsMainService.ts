...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/Neptor
	...
	private get oldId() {
		...
		const newKey = 'neptor.app.oldMachineId'
		...
		const oldValue = this._appStorage.get('neptor.machineId', StorageScope.APPLICATION) ?? 'NULL' // the old way of getting the key
		...
		// private get oldId() {
		// 	return this._memoStorage('neptor.app.oldMachineId', StorageTarget.MACHINE, 'NULL')
		// }
	}
	...
	private get distinctId() {
		...
		return this._memoStorage('neptor.app.machineId', StorageTarget.MACHINE, setValIfNotExist)
	}

	// just to see if there are ever multiple machineIDs per userID (instead of this, we should just track by the user's email)
	private get userId() {
		return this._memoStorage('neptor.app.userMachineId', StorageTarget.USER)
	}
	...
	async initialize() {
		...
		const { commit, version, neptorVersion, release, quality } = this._productService
		...
		console.log('User is opted out of basic Neptor metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('Neptor posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
