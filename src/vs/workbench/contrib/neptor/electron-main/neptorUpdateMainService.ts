...
export class NeptorMainUpdateService extends Disposable implements INeptorUpdateService {
	...
	async check(explicit: boolean): Promise<NeptorCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Neptor to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<NeptorCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/neptorai/binaries/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Neptor is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Neptor is up-to-date!'
					}
				}
				else {
