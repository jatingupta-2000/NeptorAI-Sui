...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, ProviderName } from './neptorSettingsTypes.js';
...
export type NeptorStaticModelInfo = { // not stateful
	// Neptor uses the information below to know how to handle each model.
...
type NeptorStaticProviderInfo = { // doesn't change (not stateful)
	...
	modelOptions: { [key: string]: NeptorStaticModelInfo };
	modelOptionsFallback: (modelName: string, fallbackKnownValues?: Partial<NeptorStaticModelInfo>) => (NeptorStaticModelInfo & { modelName: string, recognizedModelName: string }) | null;
}
...
const defaultModelOptions = {
	...
	supportsFIM: false,
	reasoningCapabilities: false,
} as const satisfies NeptorStaticModelInfo
...
} as const satisfies { [s: string]: Partial<NeptorStaticModelInfo> }




// keep modelName, but use the fallback's defaults
const extensiveModelOptionsFallback: NeptorStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {

	const lower = modelName.toLowerCase()

	const toFallback = <T extends { [s: string]: Omit<NeptorStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: NeptorStaticModelInfo & { modelName: string, recognizedModelName: string } => {
...
} as const satisfies { [s: string]: NeptorStaticModelInfo }

const anthropicSettings: NeptorStaticProviderInfo = {
...
const openAISettings: NeptorStaticProviderInfo = {
...
const xAISettings: NeptorStaticProviderInfo = {
...
const geminiSettings: NeptorStaticProviderInfo = {
...
const deepseekSettings: NeptorStaticProviderInfo = {
...
const mistralSettings: NeptorStaticProviderInfo = {
...
const groqSettings: NeptorStaticProviderInfo = {
...
} as const satisfies Record<string, NeptorStaticModelInfo>
const googleVertexSettings: NeptorStaticProviderInfo = {
...
const microsoftAzureSettings: NeptorStaticProviderInfo = {
...
const awsBedrockSettings: NeptorStaticProviderInfo = {
...
const vLLMSettings: NeptorStaticProviderInfo = {
...
const lmStudioSettings: NeptorStaticProviderInfo = {
...
const ollamaSettings: NeptorStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
...
const openaiCompatible: NeptorStaticProviderInfo = {
...
const liteLLMSettings: NeptorStaticProviderInfo = { // https://docs.litellm.ai/docs/reasoning_content
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const openRouterSettings: NeptorStaticProviderInfo = {
...
const modelSettingsOfProvider: { [providerName in ProviderName]: NeptorStaticProviderInfo } = {
