...
import { NEPTOR_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, NeptorStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './neptorSettingsTypes.js';
...
export type NeptorSettingsState = {
...
// type RealNeptorSettings = Exclude<keyof NeptorSettingsState, '_modelOptions'>
// type EventProp<T extends RealNeptorSettings = RealNeptorSettings> = T extends 'globalSettings' ? [T, keyof NeptorSettingsState[T]] : T | 'all'


export interface INeptorSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: NeptorSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: NeptorSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: NeptorStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, NeptorStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: NeptorSettingsState): NeptorSettingsState => {
...
const _validatedModelState = (state: Omit<NeptorSettingsState, '_modelOptions'>): NeptorSettingsState => {
...
export const INeptorSettingsService = createDecorator<INeptorSettingsService>('NeptorSettingsService');
class NeptorSettingsService extends Disposable implements INeptorSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: NeptorSettingsState;
	...
	dangerousSetState = async (newState: NeptorSettingsState) => {
	...
	async readAndInitializeState() {
		...
		try {
			...
			if (readS.globalSettings.memoryConsolidationMode === undefined) readS.globalSettings.memoryConsolidationMode = defaultGlobalSettings.memoryConsolidationMode
			if (readS.globalSettings.injectPmMemorySummaryInCodeChat === undefined) readS.globalSettings.injectPmMemorySummaryInCodeChat = defaultGlobalSettings.injectPmMemorySummaryInCodeChat
		}
		catch (e) {
			readS = defaultState()
		}
		...
		try {
			readS = {
				...defaultState(),
				...readS,
				globalSettings: { ...defaultGlobalSettings, ...readS.globalSettings },
	...
	private async _readState(): Promise<NeptorSettingsState> {
		const encryptedState = this._storageService.get(NEPTOR_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(NEPTOR_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: NeptorSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: NeptorStatefulModelInfo[] = [
...
registerSingleton(INeptorSettingsService, NeptorSettingsService, InstantiationType.Eager);
