...
export const builtinTools: {
	...
	pm_memory_search: {
		name: 'pm_memory_search',
		description: `Search curated Neptor project memory under .neptor/memory. Optimized for coding-agent continuity: project shape, tasks, regressions/fixes, stack, style, file coupling, APIs, ADRs, and workflow—not chat logs or personality. Returns section excerpts with artifact paths.`,
		params: {
			query: { description: 'Keywords or question to match against memory sections.' },
			max_hits: { description: 'Optional. Max hits between 1 and 20. Default 8.' },
		},
	},

	pm_memory_read: {
		name: 'pm_memory_read',
		description: `Read one memory markdown artifact or a single section. artifact_type must be one of: ${MEMORY_ARTIFACT_TYPES_CSV}. Prefer architecture, tasks, fixes, tech_stack, and code_style for implementation work.`,
		params: {
			artifact_type: { description: 'Artifact type key (underscore form, e.g. architecture, code_style, file_graph).' },
			section_id: { description: 'Optional section slug (e.g. overview). Leave empty or omit for the full artifact.' },
		},
	},
...
${directoryStr}
	...
	if (mode === 'agent') {
		...
		details.push(`For project continuity stored in Neptor memory (.neptor/memory)—architecture, tasks, fixes, stack, conventions—use pm_memory_search and pm_memory_read instead of guessing.`)
	}
...
// PM agent prompts (system messages + structured user messages) live in
// dedicated agent modules under ./agents. They are re-exported here so existing
// imports of `prompts.ts` keep working while individual agents can evolve in
// isolation.
export { neptor_memory_consolidation_systemMessage } from './agents/pmConsolidationAgent.js';
export {
	pm_workspace_bootstrap_chunkUserMessage,
	pm_workspace_bootstrap_systemMessage,
	type PmBootstrapChunkOptions,
} from './agents/pmBootstrapAgent.js';
export { pm_chat_systemMessage, type PmChatSystemMessageOptions } from './agents/pmChatAgent.js';
export { pmDiagramRepair_userMessage, type PmDiagramRepairOptions } from './agents/pmDiagramRepairAgent.js';
export { pmArtifactLifecycleSpec } from './agents/pmLifecycleHints.js';
export {
	SDLC_DIAGRAMS,
	ARTIFACT_DIAGRAMS,
	diagramSpec,
	diagramsForArtifact,
	diagramRequirementsMarkdown,
	type SdlcCategory,
	type SdlcDiagramSpec,
} from './agents/sdlcDiagramCatalog.js';
...
export const neptorPrefixAndSuffix = ({ fullFileStr, startLine, endLine }: { fullFileStr: string, startLine: number, endLine: number }) => {
...
// export const aiRegex_computeReplacementsForFile_userMessage = async ({ searchClause, replaceClause, fileURI, neptorFileService }: { searchClause: string, replaceClause: string, fileURI: URI, neptorFileService: INeptorFileService }) => {

// 	// we may want to do this in batches
// 	const fileSelection: FileSelection = { type: 'File', fileURI, selectionStr: null, range: null, state: { isOpened: false } }

// 	const file = await stringifyFileSelections([fileSelection], neptorFileService)
