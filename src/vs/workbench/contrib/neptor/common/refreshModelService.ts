...
import { INeptorSettingsService } from './neptorSettingsService.js';
import { ILLMMessageService } from './sendLLMMessageService.js';
import { Emitter, Event } from '../../../../base/common/event.js';
import { Disposable, IDisposable } from '../../../../base/common/lifecycle.js';
import { RefreshableProviderName, refreshableProviderNames, SettingsOfProvider } from './neptorSettingsTypes.js';
...
export class RefreshModelService extends Disposable implements IRefreshModelService {
	...
	constructor(
		...
		const initializeAutoPollingAndOnChange = () => {
			...
			if (!neptorSettingsService.state.globalSettings.autoRefreshModels) return

			for (const providerName of refreshableProviderNames) {

				// const { '_didFillInProviderSettings': enabled } = this.neptorSettingsService.state.settingsOfProvider[providerName]
				this.startRefreshingModels(providerName, autoOptions)

				// every time providerName.enabled changes, refresh models too, like a useEffect
				let relevantVals = () => refreshBasedOn[providerName].map(settingName => neptorSettingsService.state.settingsOfProvider[providerName][settingName])
				let prevVals = relevantVals() // each iteration of a for loop has its own context and vars, so this is ok
				disposables.add(
					neptorSettingsService.onDidChangeState(() => { // we might want to debounce this
		...
		neptorSettingsService.waitForInitState.then(() => {
			initializeAutoPollingAndOnChange()
			this._register(
				neptorSettingsService.onDidChangeState((type) => { if (typeof type === 'object' && type[1] === 'autoRefreshModels') initializeAutoPollingAndOnChange() })
			)
		})
	...
	startRefreshingModels: IRefreshModelService['startRefreshingModels'] = (providerName, options) => {
		...
		const autoPoll = () => {
			if (this.neptorSettingsService.state.globalSettings.autoRefreshModels) {
				// resume auto-polling
		...
		listFn({
			...
			onSuccess: ({ models }) => {
				...
				this.neptorSettingsService.setAutodetectedModels(
					providerName,
					models.map(model => {
				...
				if (options.enableProviderOnSuccess) this.neptorSettingsService.setSettingOfProvider(providerName, '_didFillInProviderSettings', true)
