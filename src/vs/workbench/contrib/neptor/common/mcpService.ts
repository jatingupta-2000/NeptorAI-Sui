...
import { MCPUserStateOfName } from './neptorSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('neptor-channel-mcp')
	...
	private async _initialize() {
		try {
			await this.neptorSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.neptorSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.neptorSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.neptorSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	// toggle MCP server and update isOn in Neptor settings
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		this._setMCPServerState(serverName, { status: 'loading', tools: [] })

		await this.neptorSettingsService.setMCPServerState(serverName, { isOn });
