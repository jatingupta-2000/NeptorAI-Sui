...
type NeptorModelType = {
	model: ITextModel | null;
	editorModel: IResolvedTextEditorModel | null;
};

export interface INeptorModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): NeptorModelType;
	getModelFromFsPath(fsPath: string): NeptorModelType;
	getModelSafe(uri: URI): Promise<NeptorModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const INeptorModelService = createDecorator<INeptorModelService>('neptorModelService');

class NeptorModelService extends Disposable implements INeptorModelService {
	_serviceBrand: undefined;
	static readonly ID = 'neptorModelService';
	...
	getModelSafe = async (uri: URI): Promise<NeptorModelType> => {
...
registerSingleton(INeptorModelService, NeptorModelService, InstantiationType.Eager);
