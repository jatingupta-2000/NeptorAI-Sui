...
import { reParsedToolXMLString, chat_systemMessage, pm_chat_systemMessage } from '../common/prompt/prompts.js';
import { AnthropicLLMChatMessage, AnthropicReasoning, GeminiLLMChatMessage, LLMChatMessage, LLMFIMMessage, OpenAILLMChatMessage, RawToolParamsObj } from '../common/sendLLMMessageTypes.js';
import { INeptorSettingsService } from '../common/neptorSettingsService.js';
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/neptorSettingsTypes.js';
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .neptorrules file):\n${aiInstructions}`)
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@INeptorProjectMemoryService private readonly projectMemoryService: INeptorProjectMemoryService,
		@IInstantiationService private readonly instantiationService: IInstantiationService,
	) {
		super()
	}

	// Read .neptorrules files from workspace folders
	private _getNeptorRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let neptorRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.neptorrules')
				const { model } = this.neptorModelService.getModel(uri)
				if (!model) continue
				neptorRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return neptorRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings, team rules, and .neptorrules files
	private _getCombinedAIInstructions(): string {
		const ans: string[] = [];
		const teamInstructions = this.instantiationService.invokeFunction(accessor => accessor.get(INeptorOrgCollaborationService).getTeamInstructionsSync());
		if (teamInstructions) {
			ans.push(`TEAM GUIDELINES (organization):\n${teamInstructions}`);
		}
		const globalAIInstructions = this.neptorSettingsService.state.globalSettings.aiInstructions;
		const neptorRulesFileContent = this._getNeptorRulesFileContents();

		if (globalAIInstructions) ans.push(globalAIInstructions);
		if (neptorRulesFileContent) ans.push(neptorRulesFileContent);
		return ans.join('\n\n');
	}


	// system message
	private _deriveLastUserChatContent = (chatMessages: ChatMessage[]): string | undefined => {
	...
	private _generateChatMessagesSystemMessage = async (
		...
		if (getNeptorAgentKind() === 'product') {
			let systemMessage = pm_chat_systemMessage({ workspaceFolders, openedURIs, directoryStr, activeURI, persistentTerminalIDs, chatMode, mcpTools, includeXMLToolDefinitions })
			const lastUser = chatMessages ? this._deriveLastUserChatContent(chatMessages) : undefined
			const memoryBlock = await this.projectMemoryService.getRetrievalContextForPrompt(lastUser)
			if (memoryBlock) {
		...
		if (this.neptorSettingsService.state.globalSettings.injectPmMemorySummaryInCodeChat) {
			const summary = await this.projectMemoryService.getSummaryForCodeChat()
			if (summary.trim()) {
				return `${systemMessage}\n\n\n<pm_memory_summary>\n${summary}\n</pm_memory_summary>`
			}
		}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.neptorSettingsService.state
		...
		const modelSelectionOptions = this.neptorSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.neptorSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat, chatMessages)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.neptorSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
