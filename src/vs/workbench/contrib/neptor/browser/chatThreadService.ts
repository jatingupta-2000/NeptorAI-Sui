...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/neptorSettingsTypes.js';
...
import { NeptorFileSnapshot } from '../common/editCodeServiceTypes.js';
import { INotificationService, Severity } from '../../../../platform/notification/common/notification.js';
import { truncate } from '../../../../base/common/strings.js';
import { THREAD_STORAGE_KEY } from '../common/storageKeys.js';
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js';
import { getNeptorAgentKind } from './neptorAgentKind.js';
...
export const IChatThreadService = createDecorator<IChatThreadService>('neptorChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	constructor(
		...
		@INeptorMemoryConsolidationService private readonly _memoryConsolidationService: INeptorMemoryConsolidationService,
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart Neptor)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart Neptor), cancel that last tool
	...
	private _lastPairForMemoryConsolidation(threadId: string): { user: string; assistant: string } {
		const messages = this.state.allThreads[threadId]?.messages ?? [];
		let assistant = '';
		let user = '';
	...
	/** Recent successful tool strings for consolidation prompts (newest last). */
	private _recentToolResultsDigest(threadId: string, maxTools: number, maxCharsPerTool: number): string {
		const messages = this.state.allThreads[threadId]?.messages ?? [];
		const chunks: string[] = [];
		for (let i = messages.length - 1; i >= 0 && chunks.length < maxTools; i -= 1) {
			const m = messages[i];
			if (m.role === 'tool' && m.type === 'success') {
				const slice = (m.content ?? '').slice(0, maxCharsPerTool).trim();
				if (slice.length > 0) {
					chunks.push(`${m.name}: ${slice}`);
				}
			}
		}
		chunks.reverse();
	...
	private async _runChatAgent({
		...
		if (!isRunningWhenEnd && getNeptorAgentKind() === 'product') {
			const { user, assistant } = this._lastPairForMemoryConsolidation(threadId);
			this._memoryConsolidationService.scheduleAfterPmAssistantMessage({ lastUserMessage: user, assistantText: assistant });
		}

		if (!isRunningWhenEnd && chatMode === 'agent' && getNeptorAgentKind() === 'code') {
			const { user, assistant } = this._lastPairForMemoryConsolidation(threadId);
			const toolDigest = this._recentToolResultsDigest(threadId, 8, 900);
			this._memoryConsolidationService.scheduleAfterCodeAgentIdle({
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const neptorFileSnapshot = checkpointMessage.neptorFileSnapshotOfURI ? checkpointMessage.neptorFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { neptorFileSnapshot, } }

		const userModifiedNeptorFileSnapshot = fsPath in checkpointMessage.userModifications.neptorFileSnapshotOfURI ? checkpointMessage.userModifications.neptorFileSnapshotOfURI[fsPath] ?? null : null
		return { neptorFileSnapshot: userModifiedNeptorFileSnapshot ?? neptorFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const neptorFileSnapshotOfURI: { [fsPath: string]: NeptorFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._neptorModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { neptorFileSnapshot: oldNeptorFileSnapshot } = res
			...
			const neptorFileSnapshot = this._editCodeService.getNeptorFileSnapshot(URI.file(fsPath))
			if (oldNeptorFileSnapshot === neptorFileSnapshot) continue
			neptorFileSnapshotOfURI[fsPath] = neptorFileSnapshot
		}
		...
		return { neptorFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { neptorFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			neptorFileSnapshotOfURI: neptorFileSnapshotOfURI ?? {},
			userModifications: { neptorFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getNeptorFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			neptorFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { neptorFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.neptorFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { neptorFileSnapshotOfURI: neptorFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { neptorFileSnapshot } = res
					if (!neptorFileSnapshot) continue
					this._editCodeService.restoreNeptorFileSnapshot(URI.file(fsPath), neptorFileSnapshot)
					break
				}
			}
		}
...
*/
	...
	private async _addUserMessageAndStreamResponse({ userMessage, _chatSelections, threadId }: { userMessage: string, _chatSelections?: StagingSelectionItem[], threadId: string }) {
		...
		this._orgCollaborationService.onUserPrompt(instructions, 'Chat');
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._neptorModelService.getModelSafe(uri)
