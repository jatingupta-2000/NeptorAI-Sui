...
import { NEPTOR_VIEW_CONTAINER_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { NEPTOR_TOGGLE_SETTINGS_ACTION_ID } from './neptorSettingsPane.js';
...
const NEPTOR_OPEN_SIDEBAR_ACTION_ID = 'neptor.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: NEPTOR_OPEN_SIDEBAR_ACTION_ID, title: localize2('neptorOpenSidebar', 'Neptor: Open Sidebar'), f1: true });
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('neptorCmdL', 'Neptor: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(NEPTOR_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(NEPTOR_OPEN_SIDEBAR_ACTION_ID)
		}
...
const NEPTOR_CMD_SHIFT_L_ACTION_ID = 'neptor.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(NEPTOR_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'neptor.settingsAction',
			title: `Neptor's Settings`,
			icon: { id: 'settings-gear' },
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(NEPTOR_TOGGLE_SETTINGS_ACTION_ID)
	}
})
