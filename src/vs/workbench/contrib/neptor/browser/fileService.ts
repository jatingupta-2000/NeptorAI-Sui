...
class FilePromptActionService extends Action2 {
	private static readonly NEPTOR_COPY_FILE_PROMPT_ID = 'neptor.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.NEPTOR_COPY_FILE_PROMPT_ID,
			title: localize2('neptorCopyPrompt', 'Neptor: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await neptorModelService.getModelSafe(uri)).model?.getLanguageId() || '',
