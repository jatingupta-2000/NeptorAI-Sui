...
import { ProviderName, SettingName, displayInfoOfSettingName, providerNames, NeptorStatefulModelInfo, customSettingNamesOfProvider, RefreshableProviderName, refreshableProviderNames, displayInfoOfProviderName, nonlocalProviderNames, localProviderNames, GlobalSettingName, featureNames, displayInfoOfFeatureName, isProviderNameDisabled, FeatureName, hasDownloadButtonsOnModelsProviderNames, subTextMdOfProviderName } from '../../../../common/neptorSettingsTypes.js'
import ErrorBoundary from '../sidebar-tsx/ErrorBoundary.js'
import { NeptorButtonBgDarken, NeptorCustomDropdownBox, NeptorInputBox2, NeptorSimpleInputBox, NeptorSwitch } from '../util/inputs.js'
...
const ButtonLeftTextRightOption = ({ text, leftButton }: { text: string, leftButton?: React.ReactNode }) => {
	...
	return <div className='flex items-center text-neptor-fg-3 px-3 py-0.5 rounded-sm overflow-hidden gap-2'>
...
const ConfirmButton = ({ children, onConfirm, className }: { children: React.ReactNode, onConfirm: () => void, className?: string }) => {
	...
	return (
		<div ref={ref} className={`inline-block`}>
			<NeptorButtonBgDarken className={className} onClick={() => {
				if (!confirm) {
					setConfirm(true);
				} else {
					onConfirm();
...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('INeptorSettingsService')
	...
	const sourcecodeOverridesLink = `https://github.com/neptorai/neptor/blob/2e5ecb291d33afbe4565921664fb7e183189c1c5/src/vs/workbench/contrib/neptor/common/modelCapabilities.ts#L146-L172`
	...
	return (
		...
		>
			{/* MODAL */}
			<div
				className="bg-neptor-bg-1 rounded-md p-4 max-w-xl w-full shadow-xl overflow-y-auto max-h-[90vh]"
			...
			>
				<div className="flex justify-between items-center mb-4">
					...
					<button
						onClick={onClose}
						className="text-neptor-fg-3 hover:text-neptor-fg-1"
					>
						<X className="size-5" />
					</button>
				</div>
				...
				<div className="text-sm text-neptor-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with Neptor, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by Neptor.`
							: `Neptor recognizes ${modelName} ("${recognizedModelName}").`}
				</div>


				{/* override toggle */}
				<div className="flex items-center gap-2 mb-4">
					<NeptorSwitch size='xs' value={overrideEnabled} onChange={setOverrideEnabled} />
					<span className="text-neptor-fg-3 text-sm">Override model defaults</span>
				</div>

				{/* Informational link */}
				{overrideEnabled && <div className="text-sm text-neptor-fg-3 mb-4">
				...
				<textarea
					key={overrideEnabled + ''}
					ref={textAreaRef}
					className={`w-full min-h-[200px] p-2 rounded-sm border border-neptor-border-2 bg-neptor-bg-2 resize-none font-mono text-sm ${!overrideEnabled ? 'text-neptor-fg-3' : ''}`}
				...
				<div className="flex justify-end gap-2 mt-4">
					<NeptorButtonBgDarken onClick={onClose} className="px-3 py-1">
						Cancel
					</NeptorButtonBgDarken>
					<NeptorButtonBgDarken
						onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	const modelDump: (NeptorStatefulModelInfo & { providerName: ProviderName, providerEnabled: boolean })[] = []
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='neptor-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='neptor-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
				...
				<div className="flex items-center gap-2 w-fit">
					...
					<div className={`w-5 flex items-center justify-center`}>
						{type === 'default' || type === 'autodetected' ? null : <button
							...
							className={`${hasOverrides ? '' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}
						>
							<X size={12} className="text-neptor-fg-3 opacity-50" />
						</button>}
					</div>
				</div>
			</div>
		...
		{showCheckmark ? (
				...
				<form className="flex items-center gap-2">
					...
					<ErrorBoundary>
						<NeptorCustomDropdownBox
							...
							className="max-w-32 mx-2 w-full resize-none bg-neptor-bg-1 text-neptor-fg-1 placeholder:text-neptor-fg-3 border border-neptor-border-2 focus:border-neptor-border-1 py-1 px-2 rounded"
			...
			<div
				className="text-neptor-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer mt-4"
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const accessor = useAccessor()
	const neptorSettingsService = accessor.get('INeptorSettingsService')
	const settingsState = useSettingsState()
	...
	const handleChangeValue = useCallback((newVal: string) => {
		neptorSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [neptorSettingsService, providerName, settingName]);
...
// 	const neptorSettingsState = useSettingsState()

// 	const needsModel = isProviderNameDisabled(providerName, neptorSettingsState) === 'addModel'

// 	// const accessor = useAccessor()
// 	// const neptorSettingsService = accessor.get('INeptorSettingsService')

// 	// const { enabled } = neptorSettingsState.settingsOfProvider[providerName]
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = neptorSettingsService.state.settingsOfProvider[providerName].enabled
// 						neptorSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [neptorSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const NeptorProviderSettings = ({ providerNames }: { providerNames: ProviderName[] }) => {
	return <>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	const enabled = neptorSettingsState.globalSettings[settingName]

	return <ButtonLeftTextRightOption
		leftButton={<NeptorSwitch
			size='xxs'
			value={enabled}
			onChange={(newVal) => {
				neptorSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	const neptorSettingsService = accessor.get('INeptorSettingsService')
	const neptorSettingsState = useSettingsState()
	return <NeptorInputBox2
		className='min-h-[81px] p-3 rounded-sm'
		initValue={neptorSettingsState.globalSettings.aiInstructions}
		...
		onChangeText={(newText) => {
			neptorSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		neptorSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [neptorSettingsService])

	return <NeptorCustomDropdownBox
		className='text-xs text-neptor-fg-3 bg-neptor-bg-1 border border-neptor-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={neptorSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-neptor-fg-3 text-sm list-decimal select-text'>
		<div className=''><ChatMarkdownRender string={`Ollama Setup Instructions`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`1. Download [Ollama](https://ollama.com/download).`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`2. Open your terminal.`} chatMessageLocation={undefined} /></div>
		<div
			className='pl-6 flex items-center w-fit'
			data-tooltip-id='neptor-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`Neptor automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	const neptorSettingsService = accessor.get('INeptorSettingsService')
	return <div
		className={`text-neptor-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer ${className}`}
		onClick={() => { neptorSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		neptorSettingsService.setGlobalSetting('autoApprove', {
			...neptorSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [neptorSettingsService, metricsService])

	return <>
		<NeptorSwitch
			size={size}
			value={neptorSettingsState.globalSettings.autoApprove[approvalType] ?? false}
			onChange={(newVal) => onToggleAutoApprove(approvalType, newVal)}
		/>
		<span className="text-neptor-fg-3 text-xs">{desc}</span>
	</>
}
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: { fromEditor?: TransferEditorType, className?: string }) => {
	...
	return <>
		<NeptorButtonBgDarken className={`max-w-48 p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	const neptorSettings = useSettingsState()
	const isOn = neptorSettings.mcpUserStateOfName[name]?.isOn

	const removeUniquePrefix = (name: string) => name.split('_').slice(1).join('_')

	return (
		<div className="border border-neptor-border-2 bg-neptor-bg-1 py-3 px-4 rounded-sm my-2">
						...
						${server.status === 'success' ? 'bg-green-500'
							: server.status === 'error' ? 'bg-red-500'
								: server.status === 'loading' ? 'bg-yellow-500'
									: server.status === 'offline' ? 'bg-neptor-fg-3'
										: ''}
					`}></div>

					{/* Server name */}
					<div className="text-sm font-medium text-neptor-fg-1">{name}</div>
				</div>
					...
					<div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
						{(server.tools ?? []).length > 0 ? (
							(server.tools ?? []).map((tool: { name: string; description?: string }) => (
								<span
									key={tool.name}
									className="px-2 py-0.5 bg-neptor-bg-2 text-neptor-fg-3 rounded-sm text-xs"

									data-tooltip-id='neptor-tooltip'
									data-tooltip-content={tool.description || ''}
									data-tooltip-class-name='neptor-max-w-[300px]'
								>
									{removeUniquePrefix(tool.name)}
								</span>
							))
						) : (
							<span className="text-xs text-neptor-fg-3">No tools available</span>
						)}
					</div>
				</div>
			)}
			...
			{isOn && server.command && (
				<div className="mt-3">
					<div className="text-xs text-neptor-fg-3 mb-1">Command:</div>
					<div className="px-2 py-1 bg-neptor-bg-2 text-xs font-mono overflow-x-auto whitespace-nowrap text-neptor-fg-2 rounded-sm">
						{server.command}
					</div>
				</div>
			)}
...
const MCPServersList = () => {
	...
	if (mcpServiceState.error) {
		content = <div className="text-neptor-fg-3 text-sm mt-2">
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'neptor-chats.json'
		}
		else if (t === 'Settings') {
			// Export user settings
			dataStr = JSON.stringify(neptorSettingsService.state, null, 2)
			downloadName = 'neptor-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					neptorSettingsService.dangerousSetState(json as any)
				}
	...
	return (
		<div className={`@@neptor-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%', overflow: 'auto' }}>
          ...
          ${selectedSection === tab
										...
										: 'bg-neptor-bg-2 hover:bg-neptor-bg-2/80 text-neptor-fg-1'}
        `}
							>
								{label}
							</button>
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`Neptor's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-neptor-fg-3 mb-2`}>{`Neptor can access any model that you host locally. We automatically detect your local models by default.`}</h3>

									<div className='opacity-80 mb-4'>
										<OllamaSetupInstructions sayWeAutoDetect={true} />
									</div>

									<NeptorProviderSettings providerNames={localProviderNames} />
								</ErrorBoundary>
							</div>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-neptor-fg-3 mb-2`}>{`Neptor can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>

									<NeptorProviderSettings providerNames={nonlocalProviderNames} />
								</ErrorBoundary>
							</div>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										<ErrorBoundary>
											...
											<div>
												...
												<div className='text-sm text-neptor-fg-3 mt-1'>
													<span>
														Experimental.{' '}
													</span>
													<span
														...
														data-tooltip-class-name='neptor-max-w-[20px]'
													>
												...
												<div className='my-2'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<NeptorSwitch
																...
																onChange={(newVal) => neptorSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
															<span className='text-neptor-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
														</div>
													</ErrorBoundary>
													...
													<ErrorBoundary>
														<div className={`my-2 ${!settingsState.globalSettings.enableAutocomplete ? 'hidden' : ''}`}>
															<ModelDropdown featureName={'Autocomplete'} className='text-xs text-neptor-fg-3 bg-neptor-bg-1 border border-neptor-border-1 rounded p-0.5 px-1' />
														</div>
													</ErrorBoundary>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-neptor-fg-3 mt-1'>Settings that control the behavior of the Apply button.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<NeptorSwitch
															...
															onChange={(newVal) => neptorSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
														<span className='text-neptor-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncApplyToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'Apply'} className='text-xs text-neptor-fg-3 bg-neptor-bg-1 border border-neptor-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
										...
										<div>
											...
											<div className='text-sm text-neptor-fg-3 mt-1'>{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<NeptorSwitch
															...
															onChange={(newVal) => neptorSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
														<span className='text-neptor-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
													</div>
												</ErrorBoundary>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<NeptorSwitch
															...
															onChange={(newVal) => neptorSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
														<span className='text-neptor-fg-3 text-xs pointer-events-none'>Auto-accept LLM changes</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-neptor-fg-3 mt-1'>{`Settings that control the visibility of Neptor suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<NeptorSwitch
															...
															onChange={(newVal) => neptorSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
														<span className='text-neptor-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-neptor-fg-3 mt-1'>Settings that control the behavior of the commit message generator.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<NeptorSwitch
															...
															onChange={(newVal) => neptorSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
														<span className='text-neptor-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncSCMToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncSCMToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'SCM'} className='text-xs text-neptor-fg-3 bg-neptor-bg-1 border border-neptor-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-neptor-fg-3 mb-4'>{`Transfer your editor settings into Neptor.`}</h4>
								...
								<div>
									...
									<h4 className='text-neptor-fg-3 mb-4'>{`Transfer Neptor's settings and chats in and out of Neptor.`}</h4>
									<div className='flex flex-col gap-8'>
										{/* Settings Subcategory */}
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											<input key={2 * s} ref={fileInputSettingsRef} type='file' accept='.json' className='hidden' onChange={handleUpload('Settings')} />
											<NeptorButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputSettingsRef.current?.click() }}>
												Import Settings
											</NeptorButtonBgDarken>
											<NeptorButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Settings')}>
												Export Settings
											</NeptorButtonBgDarken>
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { neptorSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<NeptorButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputChatsRef.current?.click() }}>
												Import Chats
											</NeptorButtonBgDarken>
											<NeptorButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Chats')}>
												Export Chats
											</NeptorButtonBgDarken>
								...
								<div>
									...
									<h4 className={`text-neptor-fg-3 mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>

									<ErrorBoundary>
										<div className='flex flex-col gap-2 justify-center max-w-48 w-full'>
											<NeptorButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openSettings') }}>
												General Settings
											</NeptorButtonBgDarken>
											<NeptorButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openGlobalKeybindings') }}>
												Keyboard Settings
											</NeptorButtonBgDarken>
											<NeptorButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.selectTheme') }}>
												Theme Settings
											</NeptorButtonBgDarken>
											<NeptorButtonBgDarken className='px-4 py-1' onClick={() => { nativeHostService.showItemInFolder(environmentService.logsHome.fsPath) }}>
												Open Logs
											</NeptorButtonBgDarken>
										</div>
									</ErrorBoundary>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-neptor-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep Neptor running smoothly. You may opt out below. Regardless of this setting, Neptor never sees your code, messages, or API keys.</h4>
									...
									<div className='my-2'>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2 my-2'>
												...
												/>
												<span className='text-neptor-fg-3 text-xs pointer-events-none'>{'Opt-out (requires restart)'}</span>
											</div>
										</ErrorBoundary>
									</div>
								</div>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-neptor-fg-3 mb-4`}>
										<ChatMarkdownRender inPTag={true} string={`
System instructions to include with all AI requests.
Alternatively, place a \`.neptorrules\` file in the root of your workspace.
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<NeptorSwitch
													...
													onChange={(newValue) => {
														neptorSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
												<span className='text-neptor-fg-3 text-xs pointer-events-none'>
													{'Disable system message'}
												</span>
											</div>
										</ErrorBoundary>
										<div className='text-neptor-fg-3 text-xs mt-1'>
											{`When disabled, Neptor will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
...
Use Model Context Protocol to provide Agent mode with more tools.
							`} chatMessageLocation={undefined} />
									...
									<div className='my-2'>
										<NeptorButtonBgDarken className='px-4 py-1 w-full max-w-48' onClick={async () => { await mcpService.revealMCPConfigFile() }}>
											Add MCP Server
										</NeptorButtonBgDarken>
									</div>
