...
import { getBasename, getRelative, neptorOpenFileFn } from '../sidebar-tsx/SidebarChat.js'
...
const Codespan = ({ text, className, onClick, tooltip }: { text: string, className?: string, onClick?: () => void, tooltip?: string }) => {
	...
	return <code
		className={`font-mono text-[0.95em] font-semibold rounded-md px-1.5 py-[1px] border border-neptor-border-3 bg-neptor-bg-1 text-[#9FC9FF] ${className}`}
		onClick={onClick}
		{...tooltip ? {
...
const CodespanWithLink = ({ text, rawText, chatMessageLocation }: { text: string, rawText: string, chatMessageLocation: ChatMessageLocation }) => {
	...
	const onClick = () => {
		if (!link) return;
		// Use the updated neptorOpenFileFn to open the file and handle selection
		if (link.selection)
			neptorOpenFileFn(link.uri, accessor, [link.selection.startLineNumber, link.selection.endLineNumber]);
		else
			neptorOpenFileFn(link.uri, accessor);
	}
...
const highlightTechnicalTokens = (text: string): React.ReactNode[] => {
	const parts: React.ReactNode[] = [];
	const tokenRegex = /(<\/?[A-Za-z][^>\n]*>|\b[\w.-]+\.(?:ts|tsx|js|jsx|py|java|go|rs|md|json|yml|yaml|css|html|sql)\b|\b[A-Za-z_]\w*\s*=\s*\{[^}\n]+\})/g;
	let last = 0;
	...
	while ((m = tokenRegex.exec(text)) !== null) {
		...
		parts.push(<Codespan key={`f-${i++}`} text={m[0]} className='' />);
...
const RenderToken = ({ token, inPTag, codeURI, chatMessageLocation, tokenIdx, ...options }: { token: Token | string, inPTag?: boolean, codeURI?: URI, chatMessageLocation?: ChatMessageLocation, tokenIdx: string, } & RenderTokenOptions): React.ReactNode => {
	...
	if (t.type === 'table') {
		...
		// return (
		// 	<div>
		// 		<table className={'min-w-full border border-neptor-bg-2'}>
		// 			<thead>
		// 				<tr className='bg-neptor-bg-1'>
		// 					{t.header.map((cell: any, index: number) => (
		// 						<th
		// 							key={index}
		// 							className='px-4 py-2 border border-neptor-bg-2 font-semibold'
		...
		// 					<tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-neptor-bg-1'}>
		// 						{row.map((cell: any, cellIndex: number) => (
		// 							<td
		// 								key={cellIndex}
		// 								className={'px-4 py-2 border border-neptor-bg-2'}
	...
	if (t.type === 'text' || t.type === 'escape' || t.type === 'html') {
		return <span>{highlightTechnicalTokens(t.raw)}</span>
	}
	...
	if (t.type === 'link') {
		return (
			<a
				...
				className='underline cursor-pointer hover:brightness-90 transition-all duration-200 text-neptor-fg-2'
			>
				{t.text}
			</a>
		)
