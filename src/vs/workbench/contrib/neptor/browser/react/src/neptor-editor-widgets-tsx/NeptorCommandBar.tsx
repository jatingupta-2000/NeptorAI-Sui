...
import { NeptorCommandBarProps } from '../../../neptorCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	NEPTOR_GOTO_NEXT_DIFF_ACTION_ID,
	NEPTOR_GOTO_PREV_DIFF_ACTION_ID,
	NEPTOR_GOTO_NEXT_URI_ACTION_ID,
	NEPTOR_GOTO_PREV_URI_ACTION_ID,
	NEPTOR_ACCEPT_FILE_ACTION_ID,
	NEPTOR_REJECT_FILE_ACTION_ID,
	NEPTOR_ACCEPT_ALL_DIFFS_ACTION_ID,
	NEPTOR_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const NeptorCommandBarMain = ({ uri, editor }: NeptorCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@neptor-scope ${isDark ? 'dark' : ''}`}
	>
		<NeptorCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const NeptorCommandBar = ({ uri, editor }: NeptorCommandBarProps) => {
	...
	const neptorModelService = accessor.get('INeptorModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(NEPTOR_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(NEPTOR_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(NEPTOR_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(NEPTOR_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(NEPTOR_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(NEPTOR_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(NEPTOR_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(NEPTOR_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-neptor-bg-2 shadow-md border border-neptor-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-neptor-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-neptor-bg-2 rounded shadow-md border border-neptor-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-neptor-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-neptor-bg-2 rounded shadow-md border border-neptor-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-neptor-border-2 [&>*:last-child]:border-r-0">
