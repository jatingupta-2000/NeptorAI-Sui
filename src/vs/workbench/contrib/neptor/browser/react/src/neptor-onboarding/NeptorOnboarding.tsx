...
import { displayInfoOfProviderName, ProviderName, providerNames, localProviderNames, featureNames, FeatureName, isFeatureNameDisabled } from '../../../../common/neptorSettingsTypes.js';
import { ChatMarkdownRender } from '../markdown/ChatMarkdownRender.js';
import { OllamaSetupInstructions, OneClickSwitchButton, SettingsForProvider, ModelDump } from '../neptor-settings-tsx/Settings.js';
...
export const NeptorOnboarding = () => {

	const neptorSettingsState = useSettingsState()
	const isOnboardingComplete = neptorSettingsState.globalSettings.isOnboardingComplete || OVERRIDE_VALUE

	const isDark = useIsDark()

	const onboardingBackdropStyle: CSSProperties = isDark
		? {
			backgroundColor: 'var(--neptor-landing-bg, #000000)',
			backgroundImage: `
				radial-gradient(ellipse 130% 70% at 50% -25%, var(--neptor-landing-glow, rgba(60, 10, 10, 0.5)) 0%, transparent 55%),
				repeating-linear-gradient(0deg, transparent, transparent 47px, var(--neptor-landing-grid, rgba(255, 59, 48, 0.07)) 47px, var(--neptor-landing-grid, rgba(255, 59, 48, 0.07)) 48px),
				repeating-linear-gradient(90deg, transparent, transparent 47px, var(--neptor-landing-grid, rgba(255, 59, 48, 0.07)) 47px, var(--neptor-landing-grid, rgba(255, 59, 48, 0.07)) 48px)
			`,
		}
		: {
			backgroundColor: '#faf8f6',
			backgroundImage: `
				radial-gradient(ellipse 120% 65% at 50% -20%, rgba(255, 158, 125, 0.22) 0%, transparent 52%),
				repeating-linear-gradient(0deg, transparent, transparent 47px, rgba(255, 59, 48, 0.04) 47px, rgba(255, 59, 48, 0.04) 48px),
				repeating-linear-gradient(90deg, transparent, transparent 47px, rgba(255, 59, 48, 0.04) 47px, rgba(255, 59, 48, 0.04) 48px)
			`,
		};

	return (
		<div className={`@@neptor-scope ${isDark ? 'dark' : ''}`}>
			<div
				className={`
				...
				style={{
					...
					...onboardingBackdropStyle,
				}}
			>
				<ErrorBoundary>
					<NeptorOnboardingContent />
				</ErrorBoundary>
			</div>
		</div>
	)
...
const NeptorIcon = () => {
	...
	return <div ref={divRef} className='@@neptor-icon' />
}
...
const AddProvidersPage = ({ pageIndex, setPageIndex }: { pageIndex: number, setPageIndex: (index: number) => void }) => {
	...
	return (<div className="flex flex-col md:flex-row w-full h-[80vh] gap-6 max-w-[900px] mx-auto relative">
		{/* Left Column */}
		<div className="md:w-1/4 w-full flex flex-col gap-6 p-6 border-none border-neptor-border-2 h-full overflow-y-auto">
			{/* Tab Selector */}
		...
		<div className="flex-1 flex flex-col items-center justify-start p-6 h-full overflow-y-auto">
			...
			<div className="w-full max-w-xl mt-4 mb-10">
				...
				<div className="text-sm opacity-80 text-neptor-fg-3 my-4 w-full">{descriptionOfTab[currentTab]}</div>
			</div>
			...
			{providerNamesOfTab[currentTab].map((providerName) => (
				<div key={providerName} className="w-full max-w-xl mb-10">
					<div className="text-xl mb-2">
						...
						{providerName === 'gemini' && (
							<span
								data-tooltip-id="neptor-tooltip-provider-info"
			...
			{(currentTab === 'Local' || currentTab === 'Cloud/Other') && (
				<div className="w-full max-w-xl mt-8 bg-neptor-bg-2/50 rounded-lg p-6 border border-neptor-border-4">
					<div className="flex items-center gap-2 mb-4">
						<div className="text-xl font-medium">Models</div>
					</div>

					{currentTab === 'Local' && (
						<div className="text-sm opacity-80 text-neptor-fg-3 my-4 w-full">Local models should be detected automatically. You can add custom models below.</div>
					)}
...
// 		title:
// 			div
// 				"How would you like to use Neptor?"
// 		content:
...
const PreviousButton = ({ onClick, ...props }: { onClick: () => void } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	return (
		<button
			onClick={onClick}
			className="px-6 py-2 rounded text-neptor-fg-3 opacity-80 hover:brightness-115 duration-600 transition-all"
			{...props}
		>
			Back
		</button>
...
const OllamaDownloadOrRemoveModelButton = ({ modelName, isModelInstalled, sizeGb }: { modelName: string, isModelInstalled: boolean, sizeGb: number | false | 'not-known' }) => {
	...
	return <a
		...
		className="flex items-center justify-center text-neptor-fg-2 hover:text-neptor-fg-1"
	>
	...
	return (
		<button
			...
			className={`
				...
				text-white
				bg-[var(--neptor-landing-coral,#ff3b30)] hover:bg-[var(--neptor-landing-coral-hover,#e63e22)] shadow-lg shadow-[rgba(255,59,48,0.25)]
...
const NeptorOnboardingContent = () => {

	const isDark = useIsDark()
	const gradientWordStyle: CSSProperties = {
		backgroundImage: 'linear-gradient(90deg, var(--neptor-landing-peach, #ff9e7d) 0%, var(--neptor-landing-coral, #ff4b2b) 100%)',
	...
	const neptorMetricsService = accessor.get('IMetricsService')
	...
	const didFillInProviderSettings = selectedProviderName && neptorSettingsState.settingsOfProvider[selectedProviderName]._didFillInProviderSettings
	const isApiKeyLongEnoughIfApiKeyExists = selectedProviderName && neptorSettingsState.settingsOfProvider[selectedProviderName].apiKey ? neptorSettingsState.settingsOfProvider[selectedProviderName].apiKey.length > 15 : true
	const isAtLeastOneModel = selectedProviderName && neptorSettingsState.settingsOfProvider[selectedProviderName].models.length >= 1
	...
	const lastPagePrevAndNextButtons = <div className="max-w-[600px] w-full mx-auto flex flex-col items-end">
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					neptorSettingsService.setGlobalSetting('isOnboardingComplete', true);
					neptorMetricsService.capture('Completed Onboarding', { selectedProviderName, wantToUseOption })
				}}
				ringSize={neptorSettingsState.globalSettings.isOnboardingComplete ? 'screen' : undefined}
			>Enter the Neptor</PrimaryActionButton>
		</div>
	</div>
	...
	const detailedDescOfWantToUseOption: { [wantToUseOption in WantToUseOption]: string } = {
		...
		private: "Private-hosted so your data never leaves your computer or network. [Email us](mailto:founders@neptor.ai) for help setting up at your company.",
	...
	useEffect(() => {
		if (!neptorSettingsState.globalSettings.isOnboardingComplete) {
			setPageIndex(0)
		}
	}, [setPageIndex, neptorSettingsState.globalSettings.isOnboardingComplete])
		...
		0: <OnboardingPageShell
			content={
				<div className='flex flex-col items-center gap-8'>
					<div className={`text-5xl font-light text-center tracking-tight ${isDark ? 'text-white' : 'text-neutral-900'}`}>
						<span className={isDark ? 'text-white' : 'text-neutral-900'}>Welcome to </span>
						<span style={gradientWordStyle}>Neptor</span>
					</div>

					{/* Slice of Neptor image */}
			...
			content={
				<div>
					...
					<div className="mt-8 text-center flex flex-col items-center gap-4 w-full max-w-md mx-auto">
						<h4 className="text-neptor-fg-3 mb-4">Transfer your settings from an existing editor?</h4>
