...
import { BlockCode, TextAreaFns, NeptorCustomDropdownBox, NeptorInputBox2, NeptorSlider, NeptorSwitch, NeptorDiffEditor } from '../util/inputs.js';
import { ModelDropdown, } from '../neptor-settings-tsx/ModelDropdown.js';
import { PastThreadsList } from './SidebarThreadSelector.js';
import { NEPTOR_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { NEPTOR_OPEN_SETTINGS_ACTION_ID } from '../../../neptorSettingsPane.js';
import { ChatMode, displayInfoOfProviderName, FeatureName, isFeatureNameDisabled } from '../../../../../../../workbench/contrib/neptor/common/neptorSettingsTypes.js';
import { ICommandService } from '../../../../../../../platform/commands/common/commands.js';
import { WarningBox } from '../neptor-settings-tsx/WarningBox.js';
...
import { ToolApprovalTypeSwitch } from '../neptor-settings-tsx/Settings.js';
...
const ReasoningOptionSlider = ({ featureName }: { featureName: FeatureName }) => {
	...
	const modelSelection = neptorSettingsState.modelSelectionOfFeature[featureName]
	const overridesOfModel = neptorSettingsState.overridesOfModel
	...
	const modelSelectionOptions = neptorSettingsState.optionsOfModelSelection[featureName][providerName]?.[modelName]
	...
	if (canTurnOffReasoning && !reasoningBudgetSlider) { // if it's just a on/off toggle without a power slider
		return <div className='flex items-center gap-x-2'>
			<span className='text-neptor-fg-3 text-xs pointer-events-none inline-block w-10 pr-1'>Thinking</span>
			<NeptorSwitch
				size='xxs'
				value={isReasoningEnabled}
				onChange={(newVal) => {
					const isOff = canTurnOffReasoning && !newVal
					neptorSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff })
				}}
			/>
		</div>
	}
	...
	if (reasoningBudgetSlider?.type === 'budget_slider') { // if it's a slider
		...
		const value = isReasoningEnabled ? neptorSettingsState.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]?.reasoningBudget ?? defaultVal
			: valueIfOff
		...
		return <div className='flex items-center gap-x-2'>
			...
			<NeptorSlider
				...
				onChange={(newVal) => {
					...
					neptorSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningBudget: newVal })
				}}
			/>
			<span className='text-neptor-fg-3 text-xs pointer-events-none'>{isReasoningEnabled ? `${value} tokens` : 'Thinking disabled'}</span>
		</div>
	}
	...
	if (reasoningBudgetSlider?.type === 'effort_slider') {
		...
		const currentEffort = neptorSettingsState.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]?.reasoningEffort ?? defaultVal
		const valueIfOff = -1
		...
		return <div className='flex items-center gap-x-2'>
			...
			<NeptorSlider
				...
				onChange={(newVal) => {
					const isOff = canTurnOffReasoning && newVal === valueIfOff
					neptorSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningEffort: values[newVal] ?? undefined })
				}}
			/>
			<span className='text-neptor-fg-3 text-xs pointer-events-none'>{isReasoningEnabled ? `${currentEffortCapitalized}` : 'Thinking disabled'}</span>
		</div>
	}
...
const ChatModeDropdown = ({ className }: { className: string }) => {
	...
	const onChangeOption = useCallback((newVal: ChatMode) => {
		neptorSettingsService.setGlobalSetting('chatMode', newVal)
...
export const NeptorChatArea: React.FC<NeptorChatAreaProps> = ({
	children,
	onSubmit,
	onAbort,
	onClose,
	...
	return (
		<div
			ref={divRef}
			className={`flex flex-col relative input text-left shrink-0 ${className}`}
			style={{
				borderRadius: '10px',
				backgroundColor: 'color-mix(in srgb, var(--neptor-bg-1, #3c3c3c) 88%, black 12%)',
				border: '1px solid var(--neptor-border-2, #454545)',
				maxHeight: '80vh',
				overflowY: 'auto',
		...
		>
			...
			<div className="relative w-full">
				<div style={{ padding: '11px 12px', paddingRight: '44px' }}>
					{children}
				</div>
			...
			{showModelDropdown && (
				...
				>
					...
					{featureName === 'Chat' && <ChatModeDropdown className='text-[11px]' />}
					<ModelDropdown featureName={featureName} className='text-[11px]' />
...
export const ButtonSubmit = ({ className, disabled, ...props }: ButtonProps & Required<Pick<ButtonProps, 'disabled'>>) => {
	...
	return <button
		...
		style={{
			...
			height: '24px',
			borderRadius: '50%',
			backgroundColor: disabled ? 'rgba(140, 140, 140, 0.5)' : '#FF3B30',
...
export const ButtonStop = ({ className, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) => {
	return <button
		...
		{...props}
	>
		<IconSquare size={10} style={{ color: 'white' }} />
	</button>
}
...
export const SelectedFiles = (
	...
	const accessor = useAccessor()
	const commandService = accessor.get('ICommandService')
	const modelReferenceService = accessor.get('INeptorModelService')
					...
					>
						...
						<div
							className={`
								...
								text-xs text-nowrap
								border rounded-sm
								${isThisSelectionProspective ? 'bg-neptor-bg-1 text-neptor-fg-3 opacity-80' : 'bg-neptor-bg-1 hover:brightness-95 text-neptor-fg-1'}
								${isThisSelectionProspective
									? 'border-neptor-border-2'
									: 'border-neptor-border-1'
								}
								hover:border-neptor-border-1
							...
							onClick={() => {
								...
								if (isThisSelectionProspective) { // add prospective selection to selections
									...
									neptorOpenFileFn(selection.uri, accessor);
									...
									neptorOpenFileFn(selection.uri, accessor, selection.range);
								}
						...
						>
							...
							{selection.type === 'File' && selection.state.wasAddedAsCurrentFile && messageIdx === undefined && currentURI?.fsPath === selection.uri.fsPath ?
								<span className={`text-[8px] 'neptor-opacity-60 text-neptor-fg-4`}>
									{`(Current File)`}
								</span>
								: null
							}
...
const ToolHeaderWrapper = ({
	...
	const desc1HTML = <span
		className={`text-neptor-fg-4 text-xs italic truncate ml-2
	...
	return (<div className=''>
		<div className={`w-full border border-neptor-border-3 rounded px-2 py-1 bg-neptor-bg-3 overflow-hidden ${className}`}>
			{/* header */}
			<div className={`select-none flex items-center min-h-[24px]`}>
				<div className={`flex items-center w-full gap-x-2 overflow-hidden justify-between ${isRejected ? 'line-through' : ''}`}>
					...
					>
						...
						>
							{isDropdown && (<ChevronRight
								className={`
								text-neptor-fg-3 mr-0.5 h-4 w-4 flex-shrink-0 transition-transform duration-100 ease-[cubic-bezier(0.4,0,0.2,1)]
								${isExpanded ? 'rotate-90' : ''}
							`}
							/>)}
							<span className="text-neptor-fg-3 flex-shrink-0">{title}</span>
					...
					<div className="flex items-center gap-x-2 flex-shrink-0">
						...
						{info && <CircleEllipsis
							className='ml-2 text-neptor-fg-4 opacity-60 flex-shrink-0'
							size={14}
							data-tooltip-id='neptor-tooltip'
						...
						{isError && <AlertTriangle
							className='text-neptor-warning opacity-90 flex-shrink-0'
							size={14}
							data-tooltip-id='neptor-tooltip'
						...
						/>}
						{isRejected && <Ban
							className='text-neptor-fg-4 opacity-90 flex-shrink-0'
							size={14}
							data-tooltip-id='neptor-tooltip'
							data-tooltip-content={'Canceled'}
							data-tooltip-place='top'
						/>}
						{desc2 && <span className="text-neptor-fg-4 text-xs" onClick={desc2OnClick}>
							{desc2}
						</span>}
						{numResults !== undefined && (
							<span className="text-neptor-fg-4 text-xs ml-auto mr-1">
			...
			{<div
				className={`overflow-hidden transition-all duration-200 ease-in-out ${isExpanded ? 'opacity-100 py-1' : 'max-h-0 opacity-0'}
					text-neptor-fg-4 rounded-sm overflow-x-auto
				  `}
			//    bg-black bg-opacity-10 border border-neptor-border-4 border-opacity-50
			>
				{children}
			</div>}
		</div>
...
const EditTool = ({ toolMessage, threadId, messageIdx, content }: Parameters<ResultWrapper<'edit_file' | 'rewrite_file'>>[0] & { content: string }) => {
	...
	const desc1OnClick = () => neptorOpenFileFn(params.uri, accessor)
	const componentParams: ToolHeaderParams = { title, desc1, desc1OnClick, desc1Info, isError, icon, isRejected, }


	const editToolType = toolMessage.name === 'edit_file' ? 'diff' : 'rewrite'
	if (toolMessage.type === 'running_now' || toolMessage.type === 'tool_request') {
		componentParams.children = <ToolChildrenWrapper className='bg-neptor-bg-3'>
			<EditToolChildren
				uri={params.uri}
				code={content}
				type={editToolType}
		...
		<div>
			<div className="w-full">
				...
				>
					{isDropdown && (
						<ChevronRight
							className={`text-neptor-fg-3 mr-0.5 h-4 w-4 flex-shrink-0 transition-transform duration-100 ease-[cubic-bezier(0.4,0,0.2,1)] ${isOpen ? 'rotate-90' : ''}`}
						/>
					)}
					<div className="flex items-center w-full overflow-hidden">
						<span className="text-neptor-fg-3">{title}</span>
					</div>
				</div>
				{/* children */}
				{<div
					className={`overflow-hidden transition-all duration-200 ease-in-out ${isOpen ? 'opacity-100' : 'max-h-0 opacity-0'} text-neptor-fg-4`}
				>
					{children}
				</div>}
			</div>
...
const UserMessageComponent = ({ chatMessage, messageIdx, isCheckpointGhost, currCheckpointIdx, _scrollToBottom }: { chatMessage: ChatMessage & { role: 'user' }, messageIdx: number, currCheckpointIdx: number | undefined, isCheckpointGhost: boolean, _scrollToBottom: (() => void) | null }) => {
	...
	if (mode === 'display') {
		...
		chatbubbleContents = <NeptorChatArea
			featureName='Chat'
			onSubmit={onSubmit}
			onAbort={onAbort}
			isStreaming={false}
	...
	return <div
		className={`
			...
			${mode === 'edit' ? 'w-full max-w-full'
				: mode === 'display' ? `self-end w-fit max-w-[90%] whitespace-pre-wrap` : ''
			}
	...
	>
		<div
			className={`text-left max-w-full ${mode === 'edit' ? '' : mode === 'display' ? 'flex flex-col overflow-x-auto cursor-pointer' : ''}`}
			style={mode === 'display' ? {
				padding: '9px 12px',
				backgroundColor: 'color-mix(in srgb, var(--neptor-accent-primary, #FF3B30) 18%, transparent 82%)',
				color: 'var(--neptor-fg-1, #cccccc)',
				borderRadius: '12px 12px 4px 12px',
				border: '1px solid color-mix(in srgb, var(--neptor-accent-primary, #FF3B30) 36%, transparent 64%)',
				fontSize: '13px',
				lineHeight: '1.5',
			} : undefined}
		...
		<div className="absolute -top-1 -right-1 z-10">
			<EditSymbol
				...
				style={{
					cursor: 'pointer',
					padding: '2px',
					backgroundColor: 'var(--neptor-bg-2, #252526)',
					border: '1px solid var(--neptor-border-3, #383838)',
					borderRadius: '50%',
					color: 'var(--neptor-fg-3, #a0a0a0)',
					opacity: isHovered || (isFocused && mode === 'edit') ? 1 : 0,
...
prose-headings:text-neptor-fg-1
prose-strong:text-neptor-fg-1
...
marker:text-neptor-fg-3

prose-blockquote:pl-3
prose-blockquote:my-3

prose-code:text-[#9FC9FF]
...
const ProseWrapper = ({ children, showAccent = false }: { children: React.ReactNode, showAccent?: boolean }) => {
	return <div
		className="relative"
		style={showAccent ? {
			paddingLeft: '12px',
...
prose-headings:text-[#FFFFFF]
prose-headings:tracking-tight
prose-h1:text-[25px] prose-h1:font-bold prose-h1:my-3
prose-h2:text-[19px] prose-h2:font-bold prose-h2:my-3
prose-h3:text-[18px] prose-h3:font-semibold prose-h3:my-2
prose-h4:text-[16px] prose-h4:font-semibold prose-h4:my-2
prose-p:text-[#E9E9E9] prose-p:leading-relaxed prose-p:my-2.5
prose-li:text-[#E4E4E4] prose-li:leading-relaxed
prose-strong:text-[#FFFFFF] prose-strong:font-semibold
prose-a:text-[#8FC4FF] hover:prose-a:underline
prose-ul:pl-5 prose-ol:pl-5 marker:text-[#B8B8B8]
prose-code:text-[#9FC9FF] prose-code:font-semibold prose-code:before:content-none prose-code:after:content-none
prose-pre:text-[#F2F2F2] prose-pre:bg-[#111214] prose-pre:border prose-pre:border-[#33373d] prose-pre:rounded-lg prose-pre:p-3
'
			style={{
				color: '#E9E9E9',
				fontSize: '15px',
...
const AssistantMessageComponent = ({ chatMessage, isCheckpointGhost, isCommitted, messageIdx }: { chatMessage: ChatMessage & { role: 'assistant' }, isCheckpointGhost: boolean, messageIdx: number, isCommitted: boolean }) => {
	...
	return <div className="flex gap-2.5 w-full min-w-0" style={{ gap: '10px' }}>
		{/* Assistant avatar */}
		<div
		...
		>
			<svg width="12" height="12" viewBox="0 0 24 24" fill="none">
				<path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" fill="#FF3B30"/>
			</svg>
		</div>
			...
			{chatMessage.displayContent &&
				<div className={`${isCheckpointGhost ? 'opacity-50' : ''}`}>
					<div
						style={{
							border: '1px solid var(--neptor-border-3, #383838)',
							backgroundColor: 'color-mix(in srgb, var(--neptor-bg-1, #3c3c3c) 88%, transparent 12%)',
							borderRadius: '10px',
							padding: '8px 10px',
						}}
					>
...
export const ListableToolItem = ({ name, onClick, isSmall, className, showDot }: { name: React.ReactNode, onClick?: () => void, isSmall?: boolean, className?: string, showDot?: boolean }) => {
	...
	>
		...
		<div className={`${isSmall ? 'italic text-neptor-fg-4 flex items-center' : ''}`}>{name}</div>
	</div>
}
...
const EditToolChildren = ({ uri, code, type }: { uri: URI | undefined, code: string, type: 'diff' | 'rewrite' }) => {
	...
	const content = type === 'diff' ?
		<NeptorDiffEditor uri={uri} searchReplaceBlocks={code} />
...
const LintErrorChildren = ({ lintErrors }: { lintErrors: LintErrorItem[] }) => {
	return <div className="text-xs text-neptor-fg-4 opacity-80 border-l-2 border-neptor-warning px-2 py-0.5 flex flex-col gap-0.5 overflow-x-auto whitespace-nowrap">
...
const BottomChildren = ({ children, title }: { children: React.ReactNode, title: string }) => {
	...
	return (
		<div className="w-full px-2 mt-0.5">
			...
			>
				<ChevronRight
					className={`mr-1 h-3 w-3 flex-shrink-0 transition-transform duration-100 text-neptor-fg-4 group-hover:text-neptor-fg-3 ${isOpen ? 'rotate-90' : ''}`}
				/>
				<span className="font-medium text-neptor-fg-4 group-hover:text-neptor-fg-3 text-xs">{title}</span>
			</div>
			<div
				className={`overflow-hidden transition-all duration-200 ease-in-out ${isOpen ? 'opacity-100' : 'max-h-0 opacity-0'} text-xs pl-4`}
			>
				<div className="overflow-x-auto text-neptor-fg-4 opacity-90 border-l-2 border-neptor-warning px-2 py-0.5">
					{children}
				</div>
			</div>
		</div>
...
const InvalidTool = ({ toolName, message, mcpServerName }: { toolName: ToolName, message: string, mcpServerName: string | undefined }) => {
	...
	componentParams.children = <ToolChildrenWrapper>
		<CodeChildren className='bg-neptor-bg-3'>
			{message}
		</CodeChildren>
	</ToolChildrenWrapper>
	return <ToolHeaderWrapper {...componentParams} />
...
const builtinToolNameToComponent: { [T in BuiltinToolName]: { resultWrapper: ResultWrapper<T>, } } = {
	'read_file': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				const { result } = toolMessage
				componentParams.onClick = () => { neptorOpenFileFn(params.uri, accessor, range) }
				...
				componentParams.children = !result.children || (result.children.length ?? 0) === 0 ? undefined
					: <ToolChildrenWrapper>
						{result.children.map((child, i) => (<ListableToolItem key={i}
							...
							onClick={() => {
								neptorOpenFileFn(child.uri, accessor)
				...
				componentParams.children = result.uris.length === 0 ? undefined
					: <ToolChildrenWrapper>
						{result.uris.map((uri, i) => (<ListableToolItem key={i}
							...
							className='w-full overflow-auto'
							onClick={() => { neptorOpenFileFn(uri, accessor) }}
						/>))}
						{result.hasNextPage &&
	...
	'read_lint_errors': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				const { result } = toolMessage
				componentParams.onClick = () => { neptorOpenFileFn(params.uri, accessor) }
	...
	'create_file_or_folder': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				...
				const { result } = toolMessage
				if (params) { componentParams.onClick = () => { neptorOpenFileFn(params.uri, accessor) } }
				componentParams.bottomChildren = <BottomChildren title='Error'>
					<CodeChildren>
						{result}
					</CodeChildren>
	...
	const fileDetailsContent = <div className="px-2 gap-1 w-full overflow-y-auto">
		{sortedCommandBarURIs.map((uri, i) => {
			...
			const fileNameHTML = <div
				className="flex items-center gap-1.5 text-neptor-fg-3 hover:brightness-125 transition-all duration-200 cursor-pointer"
				onClick={() => neptorOpenFileFn(uri, accessor)}
			>
				{/* <FileIcon size={14} className="text-neptor-fg-3" /> */}
				<span className="text-neptor-fg-3">{basename}</span>
			</div>
			...
			const detailsContent = <div className='flex px-4'>
				<span className="text-neptor-fg-3 opacity-80">{numDiffs} diff{numDiffs !== 1 ? 's' : ''}</span>
			</div>
		...
		<>
			...
			<div className='px-2'>
				<div
					className={`
						select-none
						flex w-full rounded-t-lg bg-neptor-bg-3
						text-neptor-fg-3 text-xs text-nowrap
...
const EditToolSoFar = ({ toolCallSoFar, }: { toolCallSoFar: RawToolCallObj }) => {
	...
	const desc1OnClick = () => { uri && neptorOpenFileFn(uri, accessor) }
...
export const SidebarChat = () => {
	...
	const AGENT_MODE_STORAGE_KEY = 'neptor.chat.agentMode'
	const AGENT_MODE_EVENT = 'neptor-chat-mode-change'
	const THREAD_SCOPE_KEY_PREFIX = 'neptor.chat.scope.threadId.'

	useEffect(() => {
		const onOpenNewThread = () => { chatThreadsService.openNewThread() }
		window.addEventListener(OPEN_NEW_THREAD_EVENT, onOpenNewThread)
		return () => window.removeEventListener(OPEN_NEW_THREAD_EVENT, onOpenNewThread)
	}, [chatThreadsService])

	useEffect(() => {
		const onOpenHistory = () => { commandService.executeCommand('neptor.historyAction') }
		window.addEventListener(OPEN_HISTORY_EVENT, onOpenHistory)
		return () => window.removeEventListener(OPEN_HISTORY_EVENT, onOpenHistory)
	}, [commandService])
	...
	useEffect(() => {
		...
		const activateForCurrentMode = () => {
			const mode = window.localStorage.getItem(AGENT_MODE_STORAGE_KEY) === 'product' ? 'product' : 'agent';
			const scopedKey = `${THREAD_SCOPE_KEY_PREFIX}${mode}`;
		...
		activateForCurrentMode();
		window.addEventListener(AGENT_MODE_EVENT, activateForCurrentMode);
		window.addEventListener('storage', activateForCurrentMode);
		return () => {
			window.removeEventListener(AGENT_MODE_EVENT, activateForCurrentMode);
			window.removeEventListener('storage', activateForCurrentMode);
		};
	}, [chatThreadsService]);
	...
	useEffect(() => {
		...
		const mode = window.localStorage.getItem(AGENT_MODE_STORAGE_KEY) === 'product' ? 'product' : 'agent';
		window.localStorage.setItem(`${THREAD_SCOPE_KEY_PREFIX}${mode}`, currentThreadId);
	...
	const keybindingString = accessor.get('IKeybindingService').lookupKeybinding(NEPTOR_CTRL_L_ACTION_ID)?.getLabel()
	...
	const messagesHTML = <ScrollToBottomContainer
		...
		`}
		style={{
			background: 'linear-gradient(180deg, color-mix(in srgb, var(--neptor-bg-2, #252526) 93%, transparent 7%) 0%, color-mix(in srgb, var(--neptor-bg-2, #252526) 82%, black 18%) 100%)',
		}}
	>
		{/* previous messages */}
		...
		{latestError === undefined ? null :
			<div className='px-2 my-1'>
				...
				<WarningBox className='text-sm my-2 mx-4' onClick={() => { commandService.executeCommand(NEPTOR_OPEN_SETTINGS_ACTION_ID) }} text='Open settings' />
			</div>
		}
	...
	const inputChatArea = <NeptorChatArea
		featureName='Chat'
		onSubmit={() => onSubmit()}
		onAbort={onAbort}
	...
	>
		<NeptorInputBox2
			enableAtToMention
			className={`min-h-[40px] pr-8`}
			placeholder={`Ask Neptor AI Agent...`}
	...
	const initiallySuggestedPromptsHTML = <div className='flex flex-col gap-2 w-full text-nowrap text-neptor-fg-3 select-none'>
		{[
			'Summarize my codebase',
			'How do types work in Rust?',
			'Create a .neptorrules file for me'
		].map((text, index) => (
			<div
				key={index}
	...
	const threadPageInput = <div
		...
		className="shrink-0 border-t border-neptor-border-3"
		style={{
			backgroundColor: 'color-mix(in srgb, var(--neptor-bg-2, #252526) 90%, black 10%)',
		}}
	>
		...
		<div className='pt-6 pb-2 flex flex-col items-center text-center'>
			...
			>
				<svg width='11' height='11' viewBox='0 0 24 24' fill='none' aria-hidden='true'>
					<path d='M12 3.6l1.55 4.2 4.2 1.55-4.2 1.55L12 15.1l-1.55-4.2-4.2-1.55 4.2-1.55L12 3.6z' stroke='#FF3B30' strokeWidth='1.8' strokeLinecap='round' strokeLinejoin='round' />
				</svg>
			</div>
			<div className='text-[19px] font-semibold tracking-tight text-neptor-fg-1 mb-2'>How can I help you today?</div>
			<div className='text-[15px] text-neptor-fg-3 mb-8 max-w-[640px] leading-relaxed'>Ask anything about your product, architecture, codebase, or documentation.</div>
	...
	const landingPageContent = <div
		ref={sidebarRef}
		className='w-full h-full max-h-full flex flex-col overflow-hidden px-4'
	>
		<ErrorBoundary>
			{landingPageInput}
		</ErrorBoundary>
		...
		<div className='border-t border-neptor-border-3 pt-3 pb-2 max-h-[42%] overflow-auto'>
			{Object.keys(chatThreadsState.allThreads).length > 1 ? // show if there are threads
				<ErrorBoundary>
					<div className='mb-2 text-neptor-fg-3 text-root select-none pointer-events-none'>Previous Threads</div>
					<PastThreadsList />
				</ErrorBoundary>
				:
				<ErrorBoundary>
					<div className='mb-2 text-neptor-fg-3 text-root select-none pointer-events-none'>Suggestions</div>
	...
	const threadPageContent = <div
		ref={sidebarRef}
		className='w-full h-full min-w-0 flex flex-col overflow-hidden'
	>
