...
import { FeatureName, featureNames, isFeatureNameDisabled, ModelSelection, modelSelectionsEqual, ProviderName, providerNames, SettingsOfProvider } from '../../../../../../../workbench/contrib/neptor/common/neptorSettingsTypes.js'
import { useSettingsState, useRefreshModelState, useAccessor } from '../util/services.js'
import { _NeptorSelectBox, NeptorCustomDropdownBox } from '../util/inputs.js'
import { SelectBox } from '../../../../../../../base/browser/ui/selectBox/selectBox.js'
import { IconWarning } from '../sidebar-tsx/SidebarChat.js'
import { NEPTOR_OPEN_SETTINGS_ACTION_ID, NEPTOR_TOGGLE_SETTINGS_ACTION_ID } from '../../../neptorSettingsPane.js'
import { modelFilterOfFeatureName, ModelOption } from '../../../../../../../workbench/contrib/neptor/common/neptorSettingsService.js'
...
const ModelSelectBox = ({ options, featureName, className }: { options: ModelOption[], featureName: FeatureName, className: string }) => {
	...
	const selection = neptorSettingsService.state.modelSelectionOfFeature[featureName]
	const selectedOption = selection ? neptorSettingsService.state._modelOptions.find(v => modelSelectionsEqual(v.selection, selection))! : options[0]

	const onChangeOption = useCallback((newOption: ModelOption) => {
		neptorSettingsService.setModelSelectionOfFeature(featureName, newOption.selection)
	}, [neptorSettingsService, featureName])
...
export const ModelDropdown = ({ featureName, className }: { featureName: FeatureName, className: string }) => {
	...
	const openSettings = () => { commandService.executeCommand(NEPTOR_OPEN_SETTINGS_ACTION_ID); };
