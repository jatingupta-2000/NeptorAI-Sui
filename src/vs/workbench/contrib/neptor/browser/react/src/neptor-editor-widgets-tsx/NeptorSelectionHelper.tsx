...
import '../styles.css'
import { NEPTOR_CTRL_K_ACTION_ID, NEPTOR_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { NeptorSelectionHelperProps } from '../../../../../../contrib/neptor/browser/neptorSelectionHelperWidget.js';
import { NEPTOR_OPEN_SETTINGS_ACTION_ID } from '../../../neptorSettingsPane.js';


export const NeptorSelectionHelperMain = (props: NeptorSelectionHelperProps) => {
	...
	>
		<NeptorSelectionHelper {...props} />
	</div>
}



const NeptorSelectionHelper = ({ rerenderKey }: NeptorSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(NEPTOR_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(NEPTOR_CTRL_K_ACTION_ID)

	const dividerHTML = <div className='w-[0.5px] bg-neptor-border-3'></div>
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === NEPTOR_CTRL_L_ACTION_ID || e.commandId === NEPTOR_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(NEPTOR_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(NEPTOR_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
	...
	const moreOptionsHTML = <>
		<div
			...
			'
			onClick={() => {
				commandService.executeCommand(NEPTOR_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
	...
	return <div className='
		...
		border border-neptor-border-3 bg-neptor-bg-2
