...
import { mountNeptorCommandBar } from './react/out/neptor-editor-widgets-tsx/index.js'
...
import { NEPTOR_ACCEPT_DIFF_ACTION_ID, NEPTOR_REJECT_DIFF_ACTION_ID, NEPTOR_GOTO_NEXT_DIFF_ACTION_ID, NEPTOR_GOTO_PREV_DIFF_ACTION_ID, NEPTOR_GOTO_NEXT_URI_ACTION_ID, NEPTOR_GOTO_PREV_URI_ACTION_ID, NEPTOR_ACCEPT_FILE_ACTION_ID, NEPTOR_REJECT_FILE_ACTION_ID, NEPTOR_ACCEPT_ALL_DIFFS_ACTION_ID, NEPTOR_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface INeptorCommandBarService {
...
export const INeptorCommandBarService = createDecorator<INeptorCommandBarService>('NeptorCommandBarService');
...
export class NeptorCommandBarService extends Disposable implements INeptorCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'neptor.NeptorCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._neptorModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(INeptorCommandBarService, NeptorCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type NeptorCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountNeptorCommandBar(root, accessor, { uri, editor } satisfies NeptorCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies NeptorCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('neptorAcceptDiffAction', 'Neptor: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.NeptorExtension,
			}
		});
	}
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(INeptorCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('neptorRejectDiffAction', 'Neptor: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('neptorGoToNextDiffAction', 'Neptor: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('neptorGoToPrevDiffAction', 'Neptor: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('neptorGoToNextUriAction', 'Neptor: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('neptorGoToPrevUriAction', 'Neptor: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('neptorAcceptFileAction', 'Neptor: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('neptorRejectFileAction', 'Neptor: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('neptorAcceptAllDiffsAction', 'Neptor: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: NEPTOR_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('neptorRejectAllDiffsAction', 'Neptor: Reject All Diffs in All Files'),
			keybinding: {
