...
import { neptorPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { INeptorCommandBarService } from './neptorCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { NEPTOR_ACCEPT_DIFF_ACTION_ID, NEPTOR_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { FeatureName } from '../common/neptorSettingsTypes.js';
import { INeptorModelService } from '../common/neptorModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, NeptorFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js';
import { INeptorOrgCollaborationService } from '../common/neptorOrgCollaborationService.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@INeptorModelService private readonly _neptorModelService: INeptorModelService,
		@IConvertToLLMMessageService private readonly _convertToLLMMessageService: IConvertToLLMMessageService,
		@INeptorOrgCollaborationService private readonly _orgCollaborationService: INeptorOrgCollaborationService,
	) {
		super();
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._neptorModelService.initializeModel(model.uri)
	...
	// 		message: `Neptor Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'neptor.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Neptor's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(NEPTOR_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/neptorai/void/issues/new) it.` : undefined
	// 	})
	// }
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._neptorModelService.getModel(uri)
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'neptor-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'neptor-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'neptor-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'neptor-greenBG', {
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'neptor-redBG'
	...
	private _getCurrentNeptorFileSnapshot = (uri: URI): NeptorFileSnapshot => {
		const { model } = this._neptorModelService.getModel(uri)
	...
	private _restoreNeptorFileSnapshot = async (uri: URI, snapshot: NeptorFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: NeptorFileSnapshot = this._getCurrentNeptorFileSnapshot(uri)
		let afterSnapshot: NeptorFileSnapshot | null = null
		...
		const elt: IUndoRedoElement = {
			...
			undo: async () => { opts?.onWillUndo?.(); await this._restoreNeptorFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreNeptorFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentNeptorFileSnapshot(uri)
			await this._neptorModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getNeptorFileSnapshot(uri: URI) {
		return this._getCurrentNeptorFileSnapshot(uri)
	}


	public restoreNeptorFileSnapshot(uri: URI, snapshot: NeptorFileSnapshot): void {
		this._restoreNeptorFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Neptor: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._neptorModelService.initializeModel(uri)
		await this._neptorModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Neptor: diff.type not recognized on: ${from}`)
		}
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = neptorPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Neptor: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Neptor error: ${diff}.type not recognized`)
		}
		...
		void this._orgCollaborationService.recordActivity({
			kind: 'file_edit',
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@INeptorCommandBarService private readonly _neptorCommandBarService: INeptorCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(NEPTOR_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(NEPTOR_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._neptorCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._neptorCommandBarService.onDidChangeState(e => {
