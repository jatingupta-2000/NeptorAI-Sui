...
import { INeptorSCMService } from '../common/neptorSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { INeptorSettingsService } from '../common/neptorSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/neptorSettingsTypes.js'
...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('neptorGenerateCommitMessageService');

const loadingContextKey = 'neptorSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private neptorSCM: INeptorSCMService
	...
	constructor(
		...
		this.neptorSCM = ProxyChannel.toService<INeptorSCMService>(mainProcessService.getChannel('neptor-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.neptorSCM.gitStat(path),
					this.neptorSCM.gitSampledDiffs(path),
					this.neptorSCM.gitBranch(path),
					this.neptorSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.neptorSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.neptorSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.neptorSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'NeptorSCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('neptorFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'neptor.generateCommitMessageAction',
			title: localize2('neptorCommitMessagePrompt', 'Neptor: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('neptorCommitMessagePromptTooltip', 'Neptor: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'neptor.loadingGenerateCommitMessageAction',
			title: localize2('neptorCommitMessagePromptCancel', 'Neptor: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('neptorCommitMessagePromptCancelTooltip', 'Neptor: Cancel Commit Message Generation'),
