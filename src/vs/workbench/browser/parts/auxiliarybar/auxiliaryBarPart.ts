...
const NEPTOR_DASHBOARD_MODULES_CONFIG = 'neptor.experimental.dashboardModules';
...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	override readonly minimumWidth: number = 280; // Neptor changed this (was 170)
	...
	constructor(
		...
		this._register(configurationService.onDidChangeConfiguration(e => {
			...
			} else if (e.affectsConfiguration(NEPTOR_DASHBOARD_MODULES_CONFIG)) {
				this.updateDashboardModulesClass();
			}
		}));
	}
	...
	private updateDashboardModulesClass(): void {
		const container = this.getContainer();
		if (!container) {
			return;
		}
		// Enabled by default; users can explicitly set false to opt out.
		const isEnabled = this.configurationService.getValue<boolean>(NEPTOR_DASHBOARD_MODULES_CONFIG) !== false;
		container.classList.toggle('neptor-dashboard-modules', isEnabled);
	}
	...
	private fillExtraContextMenuActions(actions: IAction[]): void {
		...
		actions.push(...[
			...
			toAction({ id: ToggleSidebarPositionAction.ID, label: currentPositionRight ? localize('move second side bar left', "Move Neptor Side Bar Left") : localize('move second side bar right', "Move Neptor Side Bar Right"), run: () => this.commandService.executeCommand(ToggleSidebarPositionAction.ID) }),
			toggleShowLabelsAction,
			toAction({ id: ToggleAuxiliaryBarAction.ID, label: localize('hide second side bar', "Hide Neptor Side Bar"), run: () => this.commandService.executeCommand(ToggleAuxiliaryBarAction.ID) })
		]);
	}
