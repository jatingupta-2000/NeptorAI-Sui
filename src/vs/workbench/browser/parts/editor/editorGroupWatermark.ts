...
/* eslint-disable */ // Neptor
import { NEPTOR_CTRL_K_ACTION_ID, NEPTOR_CTRL_L_ACTION_ID } from '../../../contrib/neptor/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		const update = async () => {
			...
			// Neptor - if the workbench is empty, show open
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				openFolderButton.root.classList.add('neptor-openfolder-button')
				...
				openSSHButton.root.classList.add('neptor-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					voidIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('neptor-link')
				...
				// show them Neptor keybindings
				const keys = this.keybindingService.lookupKeybinding(NEPTOR_CTRL_L_ACTION_ID);
				...
				const keys2 = this.keybindingService.lookupKeybinding(NEPTOR_CTRL_K_ACTION_ID);
				...
				// button3.textContent = `Neptor Settings`
				// button3.style.display = 'block'
				// button3.style.marginLeft = 'auto'
				// button3.style.marginRight = 'auto'
				// button3.classList.add('neptor-settings-watermark-button')
